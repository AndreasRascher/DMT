codeunit 90013 DMTProcessTemplateLib
{
    ...
    var
        sourceFileStorage: Record DMTSourceFileStorage;
    begin
        if processTemplateDetail."PrPl Type" in [processTemplateDetail."PrPl Type"::"Run Codeunit", processTemplateDetail."PrPl Type"::Group] then
            exit(false);
        // find source file
        if processTemplateDetail.Name = '' then
            processTemplateDetail.TestField(Name);
        sourceFileStorage.SetRange(Name, processTemplateDetail.Name);
        OK := not sourceFileStorage.IsEmpty();
    end;
